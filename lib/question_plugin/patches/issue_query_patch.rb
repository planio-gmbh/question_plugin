      ...
      def self.prepended(base) # :nodoc:
        ...
        def sql_for_field(field, operator, value, db_table, db_field, is_custom_filter=false)
          ...
          if field == "question_assigned_to_id" || field == "question_asked_by_id"
            ...
            case operator
            when "="
              sql = "#{db_table}.#{db_field} IN (" + v.collect{|val| "'#{self.class.connection.quote_string(val)}'"}.join(",") + ") AND #{db_table}.opened = true"
            when "!"
              sql = "(#{db_table}.#{db_field} IS NULL OR #{db_table}.#{db_field} NOT IN (" + v.collect{|val| "'#{self.class.connection.quote_string(val)}'"}.join(",") + ")) AND #{db_table}.opened = true"
            end
