  ...
  module Patches
    ...
    module QueriesHelperPatch
      def self.included(base) # :nodoc:
        ...
        base.class_eval do
          alias_method_chain :column_content, :questions
        end
      end
      ...
      module InstanceMethods
        def column_content_with_questions(column, issue)
          if column.name == :formatted_questions
            return format_questions(issue.questions.opened)
          else
            column_content_without_questions(column, issue)
          end
        end
        ...
        def format_questions(questions)
          ...
          questions.each do |question|
            ...
            html << "<strong>#{l(:question_text_asked_by)}</strong>: #{h question.author.to_s}<br />"
            html << "<strong>#{l(:question_text_assigned_to)}</strong>: #{h question.assigned_to.to_s}<br />"
